...
SELECT
	...
	''''+[Column_name] + ''' AS "Column name",'+ --column name
	 'i.DATA_TYPE + CASE 
WHEN DATA_TYPE IN (''char'', ''nchar'', ''varchar'', ''nvarchar'') THEN ''('' + CAST(i.CHARACTER_MAXIMUM_LENGTH AS VARCHAR(5)) + '')''
WHEN DATA_TYPE IN (''decimal'', ''numeric'')THEN ''('' + CAST(i.NUMERIC_PRECISION AS VARCHAR(5)) + '', '' + CAST(i.NUMERIC_SCALE AS VARCHAR(5)) + '')''
 ELSE ''''
  END   AS "Data type",'
...
'JOIN (SELECT SUM(CASE 
			WHEN (ISNUMERIC(cast ('+[Column_name]+' as varchar)) = 1) THEN 0
...
+' JOIN (Select  Top 1 x."Most used value" as "Most used value" , (x."% rows with most used value") as  "% rows with most used value" from (
...
GROUP BY  ' +[Column_name] + ' )x order by x."% rows with most used value" desc
...
FROM ['+@p_DatabaseName+'].['+@p_SchemaName+'].[' +[Table_Name]+ '])  AS t7 ON 1 = 1' -- subquery for calculating max and min value
	...
	'  GROUP BY  i.data_type, i.CHARACTER_MAXIMUM_LENGTH, i.NUMERIC_PRECISION, i.NUMERIC_SCALE,  t1."Count of NULL values", t2."Count of empty/zero values",t3."Only UPPERCASE strings", t4."Only LOWERCASE strings" ,
